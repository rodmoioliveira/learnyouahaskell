...
-- Tuples, however, are used when you know exactly how many values you want to combine and its type depends
-- on how many components it has and the types of the components. They are denoted with parentheses and
...
-- One way would be to use a list. That would kind of work.
...
-- But a tuple of size two (also called a pair) is its own type, which means that a list can't have a couple of pairs
-- in it and then a triple (a tuple of size three), so let's use that instead. Instead of surrounding the vectors
-- with square brackets, we use parentheses: [(1,2),(8,11),(4,5)]. What if we tried to make a shape like
...
-- You also couldn't make a list like [(1,2),("One",2)] because the first element of the list is a pair of
-- numbers and the second element is a pair consisting of a string and a number. Tuples can also be used to
-- represent a wide variety of data. For instance, if we wanted to represent someone's name and age in Haskell,
--   we could use a triple: ("Christopher", "Walken", 55). As seen in this example, tuples can also contain lists.

-- Use tuples when you know in advance how many components some piece of data should have.
-- Tuples are much more rigid because each different size of tuple is its own type, so you can't
-- write a general function to append an element to a tuple — you'd have to write a function for
-- appending to a pair, one function for appending to a triple, one function for appending to a 4-tuple, etc.

-- While there are singleton lists, there's no such thing as a singleton tuple. It doesn't really
-- make much sense when you think about it. A singleton tuple would just be the value it contains
-- and as such would have no benefit to us.

-- Like lists, tuples can be compared with each other if their components can be compared. Only
...
-- two lists that contain different types and zip them up.
...
-- If you evaluate that by typing out triangles in GHCI, you'll get a list of all possible triangles
-- with sides under or equal to 10. Next, we'll add a condition that they all have to be right triangles.
...
-- You take a starting set of solutions and then you apply transformations to
-- those solutions and filter them until you get the right ones.
