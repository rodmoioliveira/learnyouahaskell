...
-- :: is read as "has type of". Explicit types are always denoted with the first letter in capital case.
-- 'a', as it would seem, has a type of Char. It's not hard to conclude that it stands for character.
-- True is of a Bool type. That makes sense. But what's this? Examining the type of "HELLO!" yields a [Char].
-- The square brackets denote a list. So we read that as it being a list of characters.
-- Unlike lists, each tuple length has its own type.
-- So the expression of (True, 'a') has a type of (Bool, Char), whereas an expression such as ('a','b','c')
-- would have the type of (Char, Char, Char). 4 == 5 will always return False, so its type is Bool.
...
-- function from a string to a string but we did anyway. But how do we write out the type of a function that
-- takes several parameters? Here's a simple function that takes three integers and adds them together:
...
-- you can always just write the function without it and then check it with :t. Functions are expressions too,
...
-- It's denoted by single quotes. A list of characters is a string.

-- Tuples are types but they are dependent on their length as well as the types of their components,
-- so there is theoretically an infinite number of tuple types, which is too many to cover in this tutorial.
...
-- Because head takes a list of any type and returns the first element, so what could it be? Let's check!

-- ghci> :t head
-- head :: [a] -> a

-- Hmmm! What is this a? Is it a type? Remember that we previously stated that types are written in capital case,
-- so it can't exactly be a type. Because it's not in capital case it's actually a type variable.
-- That means that a can be of any type. This is much like generics in other languages, only in Haskell
-- it's much more powerful because it allows us to easily write very general functions if they don't use any
--   specific behavior of the types in them. Functions that have type variables are called polymorphic functions.
...
-- That's why we can use fst on a pair that contains any two types. Note that just because a and b are different type variables, they
-- don't have to be different types. It just states that the first component's type and the return value's type are the same.
...
-- If we want to examine its type, pass it to another function or call it as a prefix function, we have
...
-- same type and returns a Bool. The type of those two values must be a member of the Eq class (this was the class constraint).

-- The Eq typeclass provides an interface for testing for equality. Any type where it makes sense to test for equality between
-- two values of that type should be a member of the Eq class. All standard Haskell types except for IO (the type for dealing
-- with input and output) and functions are a part of the Eq typeclass.

-- The elem function has a type of (Eq a) => a -> [a] -> Bool because it uses == over a list to check whether some value we're
-- looking for is in it.

-- Some basic typeclasses:

-- Eq
-- is used for types that support equality testing. The functions its members implement are == and /=.
-- So if there's an Eq class constraint for a type variable in a function, it uses == or /= somewhere
-- inside its definition. All the types we mentioned previously except for functions are part of Eq,
...
-- ghci> :t (>)
-- (>) :: (Ord a) => a -> a -> Bool
-- All the types we covered so far except for functions are part of Ord. Ord covers all the standard
-- comparing functions such as >, <, >= and <=. The compare function takes two Ord members of the same
-- type and returns an ordering. Ordering is a type that can be GT, LT or EQ, meaning greater than, lesser
...
-- The most used function that deals with the Show typeclass is show. It takes a value whose type is a member of
...
-- What GHCI is telling us here is that it doesn't know what we want in return. Notice that in the previous uses
-- of read we did something with the result afterwards. That way, GHCI could infer what kind of result we wanted
-- out of our read. If we used it as a boolean, it knew it had to return a Bool. But now, it knows we want some type
-- that is part of the Read class, it just doesn't know which one. Let's take a look at the type signature of read.
...
-- for an expression like read "5". To see what the type is, Haskell would have to actually
-- evaluate read "5". But since Haskell is a statically typed language, it has to know all
-- the types before the code is compiled (or in the case of GHCI, evaluated). So we have to
-- tell Haskell: "Hey, this expression should have this type, in case you don't know!".

-- Enum
-- members are sequentially ordered types — they can be enumerated. The main advantage of
-- the Enum typeclass is that we can use its types in list ranges. They also have defined
-- successors and predecesors, which you can get with the succ and pred functions. Types in
-- this class: (), Bool, Char, Ordering, Int, Integer, Float and Double.
...
-- Num
-- is a numeric typeclass. Its members have the property of being able to act like numbers. Let's examine the type of a number.
...
-- ghci> :t (*)
-- (*) :: (Num a) => a -> a -> a
-- It takes two numbers of the same type and returns a number of that type. That's why (5 :: Int) * (6 :: Integer) will
-- result in a type error whereas 5 * (6 :: Integer) will work just fine and produce an Integer because 5 can act like an Integer or an Int.

-- To join Num, a type must already be friends with Show and Eq.

-- Integral is also a numeric typeclass. Num includes all numbers, including real numbers and integral numbers,
...
-- For instance, the length function has a type declaration of length :: [a] -> Int instead of having a more
-- general type of (Num b) => length :: [a] -> b. I think that's there for historical reasons or something,
-- although in my opinion, it's pretty stupid. Anyway, if we try to get a length of a list and then add it to 3.2,
-- we'll get an error because we tried to add together an Int and a floating point number. So to get around this,
